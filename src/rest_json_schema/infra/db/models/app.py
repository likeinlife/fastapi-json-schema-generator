...
class AppModel(Base):
    ...
    kind: Mapped[str] = mapped_column(kw_only=True)
    ...
    specification: Mapped[Mapping] = mapped_column(type_=JSONB, kw_only=True)
    settings: Mapped[Mapping] = mapped_column(type_=JSONB, kw_only=True)
